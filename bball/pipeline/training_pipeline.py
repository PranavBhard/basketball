...
Usage:
    python -m bball.pipeline.training_pipeline nba
    python -m bball.pipeline.training_pipeline cbb --workers 16
    python -m bball.pipeline.training_pipeline nba --season 2024-2025
"""
...
from bball.pipeline.config import PipelineConfig, TrainingConfig
...
class PointsModelPredictor:
    ...
    def _load_selected_model(self):
        ...
        from bball.data.models import PointsConfigRepository
...
Examples:
    ...
    python -m bball.pipeline.training_pipeline cbb --workers 16
    python -m bball.pipeline.training_pipeline nba --season 2024-2025
    python -m bball.pipeline.training_pipeline cbb --no-player
    python -m bball.pipeline.training_pipeline nba --features "elo_rating|none|raw|home,elo_rating|none|raw|away"
    python -m bball.pipeline.training_pipeline nba --add --features "col1,col2"  # Update columns only
    python -m bball.pipeline.training_pipeline nba --add --season 2024-2025     # Replace season rows
    python -m bball.pipeline.training_pipeline nba --add --seasons "2023-2024,2024-2025"
        """
    )
